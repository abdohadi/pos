...
class ManageUsersTest extends TestCase
{
    ...
    */
    ...
    /** @test */
    public function there_is_no_route_for_viewing_a_user()
    {
        $user = $this->createUser('admin', ['read_categories']);

        $newUser = $this->createUser('admin', ['read_users']);
        
        $this->actingAs($user)->get('dashboard/categories/{$newUser->id}')->assertStatus(404);
    }
    ...
    /** @test */
    public function creating_a_user_permission_validation()
    {
        $user = $this->createUser('admin', ['create_users']);

        // required
        $newUser = factory('App\User')->raw(['permissions'=>[]]);
        $this->actingAs($user)->post('dashboard/users', $newUser)->assertSessionHasErrors('permissions');
    }
    ...
    public function a_user_with_create_permission_can_create_users_with_a_role_and_permissions() 
    {
        ...
        // $this->assertTrue($newUser->can('read_users'));      // error
    }

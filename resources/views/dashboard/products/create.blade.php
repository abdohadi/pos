					...
					<form action="{{ route('dashboard.products.store') }}" method="post" enctype="multipart/form-data">
						...
						@foreach (config('translatable.locales') as $locale)
							...
							<div class="form-group">
								...
								<textarea name="{{ $locale }}[description]" id="description" class="form-control ckeditor" required>{{ old($locale. '.description') }}</textarea>
							</div>
						@endforeach
						...
						<div class="form-group">
							<img src="{{ asset('uploads/product_images/default.png') }}" class="img-thumbnail image-preview" style="width: 100px">
						</div>
						...
						<div class="form-group">
							...
							<input type="number" min="1" name="stock" id="stock" class="form-control" value="{{ old('stock') }}" required>
						</div>
